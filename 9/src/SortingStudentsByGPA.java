...
public class SortingStudentsByGPA  {
    private static void swap(ArrayList<Comparable> source, int first, int
            second){
        Comparable temp = source.get(first);
        source.set(first, source.get(second));
        source.set(second, temp);
    }
    public static void quickSort(ArrayList<Comparable> source, Comparator
            comp){
        quickSort(source, comp, 0, source.size()-1);
    }
    public static void quickSort(ArrayList<Comparable> source, Comparator
            comp, int leftBorder, int rightBorder) {
    ...
    public static void main(String[] args) {
        ...
        students.add(new Student("Александров Александр Александрович",
                4.3));
